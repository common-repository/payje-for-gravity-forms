...
class PayJe_GF_Gateway extends GFPaymentAddOn {

    protected $_version = PAYJE_GF_VERSION;
    protected $_min_gravityforms_version = '1.8.12';
    protected $_slug = 'gravityformspayje';
    protected $_path = 'gravityformspayje/payje.php';
    protected $_full_path = __FILE__;
    protected $_title = 'PayJe for Gravity Forms';
    protected $_short_title = 'PayJe';
    ...
    public function init() {
        ...
        add_action( 'wp_ajax_payje_gf_retrieve_api_credentials', array( $this, 'retrieve_api_credentials' ) );
        add_action( 'wp_ajax_payje_gf_set_webhook', array( $this, 'set_webhook' ) );
    ...
    public function get_menu_icon() {
        return PAYJE_URL . 'assets/images/logo-payje-small.svg';
    }
    ...
    public function scripts() {
        ...
        $scripts = array(
            ...
            array(
                'handle'  => 'payje-gf-admin',
                'src'     => PAYJE_GF_URL . 'assets/js/admin.js',
                'version' => PAYJE_GF_VERSION,
    ...
    public function localize_admin_script() {
        ...
        wp_localize_script( 'payje-gf-admin', 'payje_gf_retrieve_api_credentials', array(
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce'    => wp_create_nonce( 'payje_gf_retrieve_api_credentials_nonce' ),
        ) );

        wp_localize_script( 'payje-gf-admin', 'payje_gf_set_webhook', array(
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce'    => wp_create_nonce( 'payje_gf_set_webhook_nonce' ),
        ) );
    ...
    public function retrieve_api_credentials() {
        ...
        check_ajax_referer( 'payje_gf_retrieve_api_credentials_nonce', 'nonce' );

        $nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( $_POST['nonce'] ) : null;
        $business_id = isset( $_POST['business_id'] ) ? sanitize_text_field( $_POST['business_id'] ) : null;

        if ( !wp_verify_nonce( $nonce, 'payje_gf_retrieve_api_credentials_nonce' ) ) {
        ...
        $business = payje_gf_get_business( $business_id );
    ...
    // Set Gravity Forms webhook URL in PayJe
    public function set_webhook() {

        check_ajax_referer( 'payje_gf_set_webhook_nonce', 'nonce' );

        $nonce          = isset( $_POST['nonce'] ) ? sanitize_text_field( $_POST['nonce'] ) : null;
        $business_id    = isset( $_POST['business_id'] ) ? sanitize_text_field( $_POST['business_id'] ) : null;
        $integration_id = isset( $_POST['integration_id'] ) ? sanitize_text_field( $_POST['integration_id'] ) : null;

        if ( !wp_verify_nonce( $nonce, 'payje_gf_set_webhook_nonce' ) ) {
            wp_send_json_error( array(
                'message' => __( 'Invalid nonce', 'payje' ),
            ), 400 );
        }
    ...
    public function feed_settings_fields() {
        ...
        array_unshift( $businesses, array(
            'label' => $businesses ? __( 'Select a business', 'payje-gf' ) : __( 'No business found', 'payje-gf' ),
            'value' => '',
        ) );
        ...
        $fields = array(
            array(
                'name'       => 'business',
                'label'      => esc_html__( 'Business Selection', 'payje-gf' ),
                'type'       => 'select',
                'required'   => true,
                'choices'    => $businesses,
            ),
            ...
            array(
                'name'       => 'environment',
                'label'      => esc_html__( 'Environment', 'payje-gf' ),
                'type'       => 'radio',
                'required'   => true,
                'horizontal' => true,
                'choices'    => array(
                    array(
                        'label' => __( 'Production', 'payje-gf' ),
                        'value' => 'production',
                    ),
                    array(
                        'label' => __( 'Sandbox', 'payje-gf' ),
                        'value' => 'sandbox',
                    ),
                ),
            ),
            array(
                'name'       => 'api_key',
                'label'      => esc_html__( 'API Access Key', 'payje-gf' ),
                'type'       => 'text',
                'class'      => 'medium',
                'required'   => true,
                'readonly'   => true,
            ...
            array(
                'name'       => 'signature_key',
                'label'      => esc_html__( 'API Signature Key', 'payje-gf' ),
                'type'       => 'text',
                'class'      => 'medium',
                'required'   => true,
                'readonly'   => true,
            ...
            array(
                'name'       => 'webhook',
                'label'      => esc_html__( 'Webhook', 'payje-gf' ),
                'type'       => 'html',
                'html'       => '<p>' . __( 'Save Gravity Forms webhook URL in PayJe to receive payment notification.', 'payje-gf' ) . '</p><button type="button" id="set-webhook" class="primary button">' . esc_html__( 'Set Webhook', 'payje-gf' ) . '</button>',
            ),
        );
    ...
    public function billing_info_fields() {
        ...
        return array(
            array(
                'name'     => 'email',
                'label'    => __( 'Email', 'payje-gf' ),
                'required' => true,
            ),
            array(
                'name'     => 'phone',
                'label'    => __( 'Phone', 'payje-gf' ),
                'required' => true,
            ),
        );
    ...
    private function init_api( $data ) {
        ...
        if ( !$this->payje ) {
            ...
            $debug         = defined( 'PAYJE_GF_API_DEBUG' ) ? PAYJE_GF_API_DEBUG : false;

            $this->payje = new PayJe_GF_API();

            $this->payje->set_api_key( $api_key );
            $this->payje->set_signature_key( $signature_key );
            $this->payje->set_environment( $environment );
            $this->payje->set_debug( $debug );
        }
    ...
    public function redirect_url( $feed, $submission_data, $form, $entry ) {
        ...
        try {
            ...
            $params = array(
                ...
                'description'  => sprintf( __( 'Payment for Submission #%d', 'payje-gf' ), $entry['id'] ),
    ...
    public function callback() {
        ...
        if ( !$feed || !rgar( $feed, 'is_active' ) ) {
            $this->log_debug( __METHOD__ . '(): PayJe no longer active for form #' . $entry['form_id'] );
            return false;
        }
    ...
    private function get_return_url( $form_id, $entry_id ) {
        ...
        $page_url = add_query_arg( 'payje_gf_return', base64_encode( $ids_query ), $referer_url );
    ...
    public function supported_notification_events( $form ) {
        ...
        return array(
            'complete_payment' => esc_html__( 'Payment Completed', 'payje-gf' ),
            'fail_payment'     => esc_html__( 'Payment Failed', 'payje-gf' ),
        );
    ...
    public function admin_edit_payment_status( $payment_status, $form, $entry ) {
        ...
        $input = gform_tooltip( 'payje_gravityforms_edit_payment_status', true );
        $input .= '<select id="payment_status" name="payment_status">';
        $input .= '<option value="' . $payment_status . '"selected>' . $payment_status . '</option>';
        $input .= '<option value="' . esc_html__( 'Paid', 'payje-gf' ) . '"selected>' . esc_html__( 'Paid', 'payje-gf' ) . '</option>';
        $input .= '</select>';
    ...
    public function admin_update_payment( $form, $entry_id ) {
        ...
        $note = sprintf(
            esc_html__( 'Payment details was manually updated. Payment Method: %s. Transaction ID: %s. Amount: %s. Status: %s. Date: %s.', 'payje-gf' ),
