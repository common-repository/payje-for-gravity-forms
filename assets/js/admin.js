jQuery(document).ready(function($) {
    ...
    tab.find('#business').on('change', function(e) {
        ...
        if (selected_business_id === undefined || selected_business_id === null || selected_business_id === '') {
            ...
            $.ajax({
                url: payje_gf_retrieve_api_credentials.ajax_url,
                type: 'POST',
                dataType: 'json',
                data: {
                    action: 'payje_gf_retrieve_api_credentials',
                    nonce: payje_gf_retrieve_api_credentials.nonce,
    ...
    tab.find('#set-webhook').on('click', function(e) {
        ...
        $.ajax({
            url: payje_gf_set_webhook.ajax_url,
            type: 'POST',
            dataType: 'json',
            data: {
                action: 'payje_gf_set_webhook',
                nonce: payje_gf_set_webhook.nonce,
            ...
            success: function(response) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Webhook Set!',
                    text: 'Your Gravity Forms webhook URL have been successfully saved in PayJe.',
                    timer: 3000,
                });
            },
            error: function(xhr) {
